...
export default function CheckoutPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    if (selectedGov?.zones.length && !selectedZoneId)
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        <div className="bg-slate-800 rounded-3xl p-10 shadow-lg border border-slate-700">
          ...
          <p className="text-purple-400 font-bold text-sm mb-6 break-all">
            {orderId}
          </p>
          ...
          <button
            onClick={() => router.push("/")}
            className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-2xl transition"
          >
            العودة للرئيسية
          </button>
        </div>
