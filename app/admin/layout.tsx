      ...
      <aside className={'fixed inset-y-0 right-0 z-50 w-72 bg-white border-l border-slate-200 shadow-sm flex flex-col transition-transform duration-300 ' + (sidebarOpen ? 'translate-x-0' : 'translate-x-full md:translate-x-0')}>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (
              <button key={item.title} onClick={() => { router.push(item.href); setSidebarOpen(false); }} className={'w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition ' + (isActive ? 'bg-purple-50 text-purple-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900')}>
        ...
        <div className="p-4 border-t border-slate-100 mt-auto">
          <a href="/" target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-purple-600 hover:bg-purple-50 transition font-bold text-sm border border-purple-200 mb-3">
