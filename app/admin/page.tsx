...
import { LogOut, ShieldCheck, Package, ShoppingCart, Megaphone, Bell, X, Truck, Tags } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="sticky top-0 z-40 border-b bg-white/95 backdrop-blur-xl" style={{ borderColor: "rgba(124,58,237,0.15)" }}>
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900">لوحة تحكم المتجر</h1>
              <p className="text-xs text-slate-500">المكتبة النوعية</p>
            </div>
          </div>
          <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 rounded-xl text-red-500 hover:bg-red-50 transition font-bold text-sm border border-red-200">
      ...
      <div className="max-w-5xl mx-auto px-4 py-20">
        <h2 className="text-3xl font-black text-slate-900 text-center mb-2">مرحباً بك في لوحة التحكم</h2>
        <p className="text-slate-500 text-center mb-12">اختر القسم الذي تريد إدارته</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {cards.map((card) => (
            <button
              ...
              className="group rounded-3xl p-8 text-center transition hover:scale-105 bg-white border border-slate-200 hover:border-purple-200 shadow-sm hover:shadow-lg"
              onMouseEnter={(e) => e.currentTarget.style.boxShadow = `0 0 30px ${card.color}20`}
              onMouseLeave={(e) => e.currentTarget.style.boxShadow = `0 0 0px ${card.color}00`}
            >
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5 transition" style={{ background: `${card.color}15` }}>
